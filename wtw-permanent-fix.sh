...
# ─────────────────────────────────────────────
echo "► Removing conflicting entry files..."
rm -f index.ts index.js App.tsx
rm -f app/radar.tsx
echo "  ✓ Removed index.ts, App.tsx, app/radar.tsx"

# ─────────────────────────────────────────────
# STEP 2: Fix package.json main field
# Expo Router requires main = "expo-router/entry"
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# STEP 3: Recreate source directories clean
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# STEP 4: design-system/themes/types.ts
# ZERO imports — pure TypeScript declarations
# ─────────────────────────────────────────────
cat > design-system/themes/types.ts << 'EOF'
...
# ─────────────────────────────────────────────
# STEP 5: City theme files
# Each imports ONLY from './types' — no cycles possible
# ─────────────────────────────────────────────
cat > design-system/themes/nyc.ts << 'EOF'
...
cat > design-system/themes/miami.ts << 'EOF'
import type { CityTheme } from './types';
...
cat > design-system/themes/la.ts << 'EOF'
import type { CityTheme } from './types';
...
cat > design-system/themes/atlanta.ts << 'EOF'
import type { CityTheme } from './types';
...
cat > design-system/themes/houston.ts << 'EOF'
import type { CityTheme } from './types';
...
cat > design-system/themes/nj.ts << 'EOF'
import type { CityTheme } from './types';
...
cat > design-system/themes/philly.ts << 'EOF'
import type { CityTheme } from './types';
...
cat > design-system/themes/chicago.ts << 'EOF'
import type { CityTheme } from './types';
...
cat > design-system/themes/dc.ts << 'EOF'
import type { CityTheme } from './types';
...
cat > design-system/themes/lasvegas.ts << 'EOF'
import type { CityTheme } from './types';
...
echo "  ✓ All 10 city theme files"

# ─────────────────────────────────────────────
# STEP 6: design-system/themes/index.ts
# ─────────────────────────────────────────────
cat > design-system/themes/index.ts << 'EOF'
...
# ─────────────────────────────────────────────
# STEP 7: design-system/ThemeProvider.tsx
# ─────────────────────────────────────────────
cat > design-system/ThemeProvider.tsx << 'EOF'
...
# ─────────────────────────────────────────────
# STEP 8: store/cityStore.ts
# ─────────────────────────────────────────────
cat > store/cityStore.ts << 'EOF'
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      <Image
        ...
        resizeMode="contain"
      />
      <Text style={[styles.wordmark, { color: theme.primary }]}>WTW</Text>
...
# ─────────────────────────────────────────────
# STEP 12: Placeholder tab screens
# ─────────────────────────────────────────────
...
export default function ExploreScreen() {
  ...
  return (
    <View style={[styles.c, { backgroundColor: theme.background }]}>
      <Text style={[styles.t, { color: theme.text.secondary }]}>EXPLORE</Text>
    </View>
  );
}
const styles = StyleSheet.create({
...
EOF
echo "  ✓ app/(tabs)/explore.tsx, saved.tsx, profile.tsx"
...
# ─────────────────────────────────────────────
# STEP 14: Verify package.json main field
# ─────────────────────────────────────────────
echo ""
MAIN=$(node -e "console.log(require('./package.json').main)")
if [ "$MAIN" = "expo-router/entry" ]; then
  echo "  ✓ package.json main = \"expo-router/entry\""
else
  echo "  ✗ ERROR: package.json main = \"$MAIN\" (expected \"expo-router/entry\")"
  exit 1
fi

# ─────────────────────────────────────────────
# STEP 15: Verify no old entry files exist
# ─────────────────────────────────────────────
for f in index.ts App.tsx; do
  if [ -f "$f" ]; then
    echo "  ✗ ERROR: $f still exists — remove it manually"
    exit 1
  fi
done
echo "  ✓ No conflicting entry files"
...
echo "  • WTW logo (icon.png) + gold 'WTW' wordmark"
echo "  • 4 bottom tabs: RADAR / EXPLORE / SAVED / PROFILE"
echo ""
echo "  If you have a custom logo, copy it to assets/wtw-logo.png"
echo "  and update the require() path in app/(tabs)/index.tsx"
echo ""
